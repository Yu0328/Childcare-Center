...
export async function renderFormListView(
  container,
  { child, onSelectForm, onBack, confirmDelete = message => (typeof confirm === 'function' ? confirm(message) : false) }
) {
  ...
  for (const form of forms) {
    container.querySelector(`[data-delete-form="${form.id}"]`).addEventListener('click', async () => {
      ...
      try {
        ...
        await renderFormListView(container, { child, onSelectForm, onBack, confirmDelete });
      } catch (err) {
